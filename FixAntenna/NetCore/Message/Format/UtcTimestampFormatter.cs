...
{
	internal class UtcTimestampFormatter : UtcDateFormatter
	{
		...
		protected internal override int FormatUtcCalendar(DateTimeOffset calendar, byte[] buff, int offset)
		{
			var formatOffset = base.FormatUtcCalendar(calendar, buff, offset);
			buff[formatOffset++] = (byte)'-';
			formatOffset = CalendarFormatUtil.FormatHours(calendar.Hour, buff, formatOffset);
			buff[formatOffset++] = (byte)':';
