...
{
	internal sealed class SessionThrowErrorUpTask : ITask, IFixSessionListener
	{
		...
		private const string THROW_TIMES_PARAM = "trows";
		...
		private int _port;
		private int _throwTimes = 0;
		private int _maxThrowTimes = 1;
		...
		public void Init(IDictionary<string, string> @params, CustomConcurrentDictionary<string, object> session)
		{
			...
			if (!string.ReferenceEquals(@params[THROW_TIMES_PARAM], null))
			{
				_maxThrowTimes = int.Parse(@params[THROW_TIMES_PARAM]);
			}
