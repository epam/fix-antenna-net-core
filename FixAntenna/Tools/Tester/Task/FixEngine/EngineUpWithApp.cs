...
{
	public sealed class EngineUpWithApp : ITask
	{
		private static readonly ILog _log = LogFactory.GetLog(typeof(EngineUpWithApp));
		...
		public void DoTask()
		{
			...
			server.SetListener(new ConnectAllFIXServerListener(this));
			try
			{
				server.Start();
			}
		...
		{
			private readonly EngineUpWithApp _outerInstance;

			public ConnectAllFIXServerListener(EngineUpWithApp outerInstance)
			{
			...
			public void NewFixSession(IFixSession session)
			{
				try
				{
					session.SetFixSessionListener(new DummySessionListener(this));
					session.Connect();
				}
				catch (IOException e)
				{
			...
			internal class DummySessionListener : IFixSessionListener
			{
				private readonly ConnectAllFIXServerListener _outerInstance;

				public DummySessionListener(ConnectAllFIXServerListener outerInstance)
				{
					this._outerInstance = outerInstance;
				}
