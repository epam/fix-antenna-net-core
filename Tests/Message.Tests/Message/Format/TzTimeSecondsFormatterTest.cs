...
{
	[TestFixture]
	internal class TzTimeSecondsFormatterTest : GenericDateFormatterTst
	{
		internal TzTimeSecondsFormatter FixDateFormatter = new TzTimeSecondsFormatter();
		...
		public virtual void TestFormattedStringLengthForHourTz()
		{
			...
			ClassicAssert.AreEqual("HH:MM:SS-hh".Length, FixDateFormatter.GetFormattedStringLength(cal),
				"Wrong format length");
		}
		...
		public virtual void TestFormattedStringLengthForMinTz()
		{
			...
			ClassicAssert.AreEqual("HH:MM:SS-hh:mm".Length, FixDateFormatter.GetFormattedStringLength(cal),
				"Wrong format length");
		}
		...
		public virtual void TestFormattedStringLengthForUtc()
		{
			...
			ClassicAssert.AreEqual("HH:MM:SSZ".Length, FixDateFormatter.GetFormattedStringLength(cal), "Wrong format length");
		}
		...
		{
			var cal = CalendarHelper.GetUtcShiftedTestCalendar(TimeSpan.FromHours(-2));
			CheckFormat(FixDateFormatter, cal, "23:59:59-02");
		}
		...
		{
			var cal = CalendarHelper.GetUtcShiftedTestCalendar(TimeSpan.FromMinutes(-150));
			CheckFormat(FixDateFormatter, cal, "23:59:59-02:30");
		}
		...
		{
			var cal = CalendarHelper.GetUtcTestCalendar();
			CheckFormat(FixDateFormatter, cal, "23:59:59Z");
		}
	}
}