...
{
	internal class FixMessageUtcMonthYearOperationsTest : AbstractGetTagValueAsXxxDateValidator
	{
		private static readonly string[] Dates = { "20130101", "20131231", "201003w4" };
		...
		{
			FixDateFormatterFactory.FixDateType.MonthYearWithDate,
			FixDateFormatterFactory.FixDateType.MonthYearWithDate,
			FixDateFormatterFactory.FixDateType.MonthYearWithWeek
		};

		public FixMessageUtcMonthYearOperationsTest()
		{
			...
			UtcCalendars[2] = new DateTimeOffset(2010, 3, 22, 0, 0, 0, TimeSpan.Zero);
		}
		...
		{
			var actual = ffl.GetTagValueAsMonthYear(tagId);
		...
		{
			var actual = ffl.GetTagValueAsMonthYear(tagId, occurrence);
		...
		{
			DateTimeOffset actual = ffl.GetTagValueAsMonthYearAtIndex(firstTagIndex);
			ClassicAssertCalendarsEquals(occurrence, actual);
		}
