...
{
	internal class MmfPersistentInMemoryQueueTest
	{
		private const string Filename = "testqueue.bin";
		private FixMessage _list;
		private MmfPersistentInMemoryQueue<FixMessageWithType> _queue;
		...
		{
			var utils = new MmfPersistentInMemoryQueueUtils();
		...
		{
			var utils = new MmfPersistentInMemoryQueueUtils();
			ClassicAssert.AreEqual(StringHelper.NewString(new byte[] { 0 }), StringHelper.NewString(utils.GetLengthHeader(0)));
		}
		...
		{
			_queue = new MmfPersistentInMemoryQueue<FixMessageWithType>(Filename, new FixMessageWithTypeFactory());
			_queue.Initialize();
		}
	}
}