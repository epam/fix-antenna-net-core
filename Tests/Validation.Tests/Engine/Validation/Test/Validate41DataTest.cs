...
{
	[TestFixture]
	internal class Validate41DataTest : GenericValidationTestStub
	{
		[TearDown]
		...
		private static readonly ILog Log = LogFactory.GetLog(typeof(Validate41DataTest));
		private const bool ErrorShouldOccur = false;

		public override FixInfo GetFixInfo()
		{
			return new FixInfo(FixVersion.Fix41);
		}

		[Test]
		[TestCaseSource(nameof(GetResourcePaths), new object[] { "fixdatafix41" })]
		...
		[Test]
		[TestCaseSource(nameof(GetResourcePaths), new object[] { "FIX41" })]
