...
{
	[TestFixture]
	internal class Validate43DataTest : GenericValidationTestStub
	{
		[TearDown]
		...
		private static readonly ILog Log = LogFactory.GetLog(typeof(Validate43DataTest));
		private const bool ErrorShouldOccur = false;

		public override FixInfo GetFixInfo()
		{
			return new FixInfo(FixVersion.Fix43);
		}

		[Test]
		[TestCaseSource(nameof(GetResourcePaths), new object[] { "fixdatafix43" })]
		...
		[Test]
		[TestCaseSource(nameof(GetResourcePaths), new object[] { "FIX43" })]
